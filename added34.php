...
<div id=ce2>
 <div id="ap"><img src="素材/上架3.png" width="978" height="557" alt="上架3-1" />
   <div id="apDiv">
<FORM name="form1" action="added_04.php?id=<?PHP echo $_GET['id'];?>&gender=<?PHP echo $_GET['gender'];?>&s_number=<?PHP echo $_GET['s_number'];?>" method="post" enctype="multipart/form-data" >
<?PHP
include("loginConfirm.php"); 
include("server.php");
$_SESSION['Specification'] = "4";
if($_SESSION['added'] == "1")
{
	?><script>window.alert('商品已存入無法修改');
	window.location="added_44.php"; 
	</script>
    <?PHP
	die();
...
<tr>
    ...
    <td rowspan="13" style="text-align:right">
...
<tr>
    ...
    <td>
    	<FONT SIZE=2>上下高度 </FONT><input type="text" id="UDHeight" name="UDHeight" size="5"><FONT SIZE=2> cm　　　</FONT>
        <FONT SIZE=2> 左右寬度 </FONT><input type="text" id="LRLength" name="LRLength" size="5"><FONT SIZE=2> cm　　　</FONT>
        <FONT SIZE=2> 底部寬度 </FONT><input type="text" id="bWidth" name="bWidth" size="5"><FONT SIZE=2> cm　　　</FONT>
     </td>
</tr>
<tr>
     <td>
        <FONT SIZE=2> 提把寬度 </FONT><input type="text" id="MWidth" name="MWidth" size="5"><FONT SIZE=2> cm　　　</FONT>
        <FONT SIZE=2> 背帶最長 </FONT><input type="text" id="SLongest" name="SLongest" size="5"><FONT SIZE=2> cm　　　</FONT>
    </td>
</tr>

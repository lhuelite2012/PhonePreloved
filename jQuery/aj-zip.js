...
var zone=[
    //市
    ['台北市', '基隆市','新北市','宜蘭縣','新竹市','新竹縣','桃園縣','苗栗縣','台中市','彰化縣','南投縣','嘉義市','嘉義縣','雲林縣','台南市','高雄市','屏東縣','台東縣','花蓮縣','澎湖縣','金門縣','連江縣','南海島','釣魚台'],
    ...
    //桃園縣
    ['中壢市','平鎮市','龍潭鄉','楊梅鎮','新屋鄉','觀音鄉','桃園市','龜山鄉','八德市','大溪鎮','復興鄉','大園鄉','蘆竹鄉'],
    //苗栗縣
    ['竹南鎮','頭份鎮','三灣鄉','南庄鄉','獅潭鄉','後龍鎮','通霄鎮','苑裡鎮','苗栗市','造橋鄉','頭屋鄉','公館鄉','大湖鄉','泰安鄉','銅鑼鄉','三義鄉','西湖鄉','卓蘭鎮'],
    //台中市
    ['中區','東區','南區','西區','北區','北屯區','西屯區','南屯區','大平區','大里區','霧峰區','烏日區','豐原區','后里區','石岡區','東勢區','和平區','新社區','潭子區','大雅區','神岡區','大肚區','沙鹿區','龍井區','梧棲區','清水區','大甲區','外埔區','大安區'],
    //彰化縣
    ['彰化市','芬園鄉','花壇鄉','秀水鄉','鹿港鎮','福興鄉','線西鄉','和美鎮','伸港鄉','員林鎮','社頭鄉','永靖鄉','埔心鄉','溪湖鎮','大村鄉','埔鹽鄉','田中鎮','北斗鎮','田尾鄉','埤頭鄉','溪州鄉','竹塘鄉','二林鎮','大城鄉','芳苑鄉','二水鄉'],
    //南投縣
    ...
    //台南市
    ['中西區','東區','南區','北區','安平區','安南區','永康區','歸仁區','新化區','左鎮區','玉井區','楠西區','南化區','仁德區','關廟區','龍崎區','官田區','麻豆區','佳里區','西港區','七股區','將軍區','學甲區','北門區','新營區','後壁區','白河區','東山區','六甲區','下營區','枊營區','鹽水區','善化區','大內區','山上區','新市區','安定區'],
    //高雄市
    ['新興區','前金區','苓雅區','鹽埕區','鼓山區','旗津區','前鎮區','三民區','楠梓區','小港區','左營區','仁武區','大社區','岡山區','路竹區','阿蓮區','田寮區','燕巢區','橋頭區','梓官區','彌陀區','永安區','湖內區','鳳山區','大寮區','林園區','鳥松區','大樹區','旗山區','美濃區','六龜區','內門區','杉林區','甲仙區','桃源區','那瑪夏','茂林區','茄萣區'],
    //屏東縣
    ['屏東市','三地門','霧台鄉','瑪家鄉','九如鄉','里港鄉','高樹鄉','鹽埔鄉','長治鄉','麟洛鄉','竹田鄉','內埔鄉','萬丹鄉','潮州鎮','泰武鄉','來義鄉','萬巒鄉','崁頂鄉','新埤鄉','南州鄉','林邊鄉','東港鎮','琉球鄉','佳冬鄉','新園鄉','枋寮鄉','枋山鄉','春日鄉','獅子鄉','車城鄉','牡丹鄉','恆春鎮','滿州鄉'],
    //台東縣
...
var zip=[
    ...
    //台中市
    ['','400','401','402','403','404','406','407','408','411','412','413','414','420','421','422','423','424','426','427','428','429','432','433','435','436','437','438','439'],
    //彰化縣
...
$(function() {
	...
	z1 = "zoneA1";     //設定HTML的"市"元件的name
    z2 = "zoneA2";     //設定HTML的"縣"元件的name
    ...
    for(var i in zone[0]){
        $("select[name=" + z1 + "]").
            ...
            attr("value", parseInt(i) + parseInt(1)).
            text(zone[0][i]));        
	}
    
    $("select[name=" + z1 + "]").change(function(){
        $("select[name=" + z2 + "]").children().remove();
        addNone(z2);
        
        if($(this).val() > 0){
            for(var i in zone[$(this).val()]){
                $("select[name=" + z2 + "]").
                    append($("<option></option>").
                    attr("value", parseInt(i) + parseInt(1)).
                    text(zone[$(this).val()][i]));                
    		}
        }        
    });
    
    $("select[name=" + z2 + "]").change(function(){
        if($(this).val() > 0){
            $("#zip").text(zip[$("select[name=" + z1 + "]").val()][$("select[name=" + z2 + "]").val()]);            
        }
    });
    ...
    $('#btn').click(function(){
        ...
        for(i = 1; i < zip.length; i++){        
            for(j = 1; j < zip[i].length; j++){    
                if(zip[i][j] == z){
                    $("#zone1").text(zone[0][i - 1]);
                    $("#zone2").text(zone[i][j - 1]);
                }
            }
        }
    });

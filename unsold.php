...
<?	
	...
	//查詢商品名稱為未賣出
	$m_number = $list3[0];
	$sql_c = "select * from commodity where m_number = '$m_number' and orsell = '0' and orend = '1'";
...
<?	
	while($list3_c = mysql_fetch_row($query_c))
	{
		...
		//出價次數
		$sql_bc = "select count(*) from bid where c_number = '$c_number'";
		$query_bc = mysql_query($sql_bc);
		$list3_bc = mysql_fetch_array($query_bc);
